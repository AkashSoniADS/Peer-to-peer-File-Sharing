...
int first(void* fd, int id)
{
	...
	array[0]=confd;
	array[1]=id;
	pthread_create(&tid,NULL,idcheck,(void *)array);//creating thread to pass the request to the requested client
	while(1)
	{
...
for(int i=0;i<20;i++)
{
                 printf("%c",clients[did-1].ipaddress[i]);
}
		write(confd,(void *)clients[did-1].ipaddress,20*sizeof(char));//sending ip_address to the requesting client 
		download(id,did);
	}
	else
	{
...
int main(void)
{
	...
	cnt=(int *)shmat(cntaddress,NULL,0);//attaches the shared memory segment to the calling process
        clients=(struct client *)shmat(listaddress,NULL,0);
	downloadstatus=(int *)shmat(daddress,NULL,0);	
	*downloadstatus=0;
	sockfd=socket(AF_INET,SOCK_STREAM,IPPROTO_TCP); //opening socket
        bzero(&servaddr,sizeof(servaddr));
        servaddr.sin_family=AF_INET;
        servaddr.sin_addr.s_addr=htonl(INADDR_ANY);
        servaddr.sin_port=htons(13000);//opening port
        bind(sockfd,(SA *) &servaddr,sizeof(servaddr));
	*cnt=0;
        listen(sockfd,LISTENQ);//listening mode
                len=sizeof(cliaddr);
                printf("\nServer Ready\n");
                while(1)
                {
			...
			printf("Connection Established.\n");//estblishing connection with the requesting client
			ret=read(confd,(void *)ip,17);//getting its ip address
			if(ret==-1) 
				printf("read error.\n");
			strncpy(clients[*cnt].ipaddress,ip,17);//storing the calling client's ipaddress in the array of ipaddresses
			printf("IP Address of Client %d connected is %s\n",(*cnt+1),clients[*cnt].ipaddress);
			strcpy(filename,"");
			clients[*cnt].id=*cnt+1;//storing the calling client's client id
			...
			{			
				...
				printf("Listing the files of Client %d.\n",(*cnt+1));
                      //storing  the files from  various clients into the array
